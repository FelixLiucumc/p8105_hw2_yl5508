...
**(1.1.1)** `pols_data` is from `pols-month.csv`.  
Break the variable `mon` into three variables `y`, `m`, `day`, then remove the first two variables.  
Variables are arranged by order of `year`, `month`, `president`, ... .
...
**(1.1.2)** `snp_data` is from `snp.csv`.
...
Variables are arranged by order of `year`, `month`, `close`.
...
**(1.1.3)** `unemployment_data` is from `unemployment.csv`.  
Reorganize the dataset by creating two new variables `month` and `unemployment`, using function `pivot_longer()`.  
Variables are arranged by order of `year`, `month`, `unemployment_rate`.
...
(a) `pols_data` contains 822 obs. of 9 variables. The key variables are `year`, `month`, `president`.
(b) `snp_data` contains 787 obs. of 4 variables. The key variables are `year`, `month`, `close`.
(c) `unemployment_data` contains 816 obs. of 3 variables. The key variables are `year`, `month`, `unemployment_rate`.

## Problem 2

### Problem 2.1 DATA CLEANING

**(2.1.1)** `mrt_data` is from sheet `Mr. Trash Wheel`.  
...
Variables are arranged by order of `sourcename`, `dumpster`, `year`, `month`, `day`, `homes_powered`, `weight_tons` ... .
...
**(2.1.2)** `pro_data` is from sheet `Professor Trash Wheel`.  
Break the variable `date` into three variables `y`, `m`, `day`, then remove the first two variables and get variable `day`.  
Calculate the `homes_powered` using data in `weight_tons`, according to the given formula.  
Create a label(column `sourcename`) for this dataset.  
Variables are arranged by order of `sourcename`, `dumpster`, `year`, `month`, `day`, `homes_powered`, `weight_tons` ... .
...
**(2.1.3)** `gwy_data` is from sheet `Gwynnda Trash Wheel`.  
Break the variable `date` into three variables `y`, `m`, `day`, then remove the first two variables and get variable `day`.  
Calculate the `homes_powered` using data in `weight_tons`, according to the given formula.  
Create a label(column `sourcename`) for this dataset.  
Variables are arranged by order of `sourcename`, `dumpster`, `year`, `month`, `day`, `homes_powered`, `weight_tons` ... .
...
(c) Total weight collected by Professor Trash Wheel is `r total_trash_data |> filter(sourcename == "Professor Trash Wheel") |> summarise(sum(weight_tons)) |> pull()` (or using another formula `r #sum(total_trash_data$weight_tons[total_data$sourcename == "Professor Trash Wheel"])`).
(d) Total number of cigarette butts collected by Gwynnda in July of 2021 is `r total_trash_data |> filter(sourcename == "Professor Trash Wheel" & year == "2021" & month == "July") |> summarise(sum(cigarette_butts)) |> pull()`.

## Problem 3

### Problem 3.1 DATA CLEANING

For dataset `MCI_baseline.csv`, we notice that there're two rows delineating info of the data. During cleaning process, we shall skip them in order to grab the raw data we want.
...
(a) There're `r nrow(bas_data)` participants recruited in the research. From the data, we notice `r nrow(bas_data_dropna)` participants develop MCI.
...
`r nrow(amyloid_only)` participants only appear in baseline dataset. IDs of these participants are shown below:
`r only_amyloid`. 
