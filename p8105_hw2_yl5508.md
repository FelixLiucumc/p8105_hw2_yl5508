...
**(1.1.2)** `snp_data` is from `snp.csv`. Use function `ifelse()` to
correct year format, then switch it back to character type.  
...
2)  `snp_data` contains 787 obs. of 4 variables. The key variables are
    `year`, `month`, `close`.
...
Variables are arranged by order of `sourcename`, `dumpster`, `year`,
...
For dataset `MCI_baseline.csv`, we notice that there’re two rows
delineating info of the data. During cleaning process, we shall skip
them in order to grab the raw data we want.
...
1)  There’re 483 participants recruited in the research. From the data,
    we notice 93 participants develop MCI.
...
80 participants only appear in baseline dataset. IDs of these
...
apoe4`,`age_at_onset`,`time`,`ratio`.   (f) The combined data has been saved as`combined_bas_amy.csv\`.
